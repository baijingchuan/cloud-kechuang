	...
	public void ajax_upload() {
		try{
			...
			if (null != up) {
				...
				url = url.replace("\\", "/");
				// 测试后缀
				boolean ok = KindEditorUtils.testExt("image", fileName);
				if (!ok) {
					ajaxResult.addError("上传文件的类型不允许");
			...
			String httpurl = basePath + url;
			JSONObject json =new JSONObject();
			json.put("httpurl", httpurl);
			jsonResult.success(json,"上传成功");
			renderJson(ajaxResult);

	    }catch(Exception e){
			ajaxResult.addError("上传发生异常");
			renderJson(ajaxResult);
			e.printStackTrace();
		}
	}
